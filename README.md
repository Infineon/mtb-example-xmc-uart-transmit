# XMC&trade; MCU: UART transmit

This code example demonstrates simple UART communication using XMC&trade; MCU by printing periodic "Hello World!!" messages on a terminal and blinking an LED. The message can be seen on a PC terminal.
...
The following example will clone the "[UART transmit](https://github.com/Infineon/mtb-example-xmc-uart-transmit)" application with the desired name "UARTTransmit" configured for the *KIT_XMC14_BOOT_001* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC14_BOOT_001 --app-id mtb-example-xmc-uart-transmit --user-app-name UARTTransmit --target-dir "C:/mtb_projects"
   ```
...
5. Confirm that the kit LED toggles every second.
...
In this code example, the UART is configured to send a "Hello World" message to the terminal at an interval of 1 second. The user LED toggles at approximately 1 Hz.
...
Document title: *CE232576* - *XMC&trade; MCU: UART transmit*
